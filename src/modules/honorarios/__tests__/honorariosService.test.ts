...
describe('honorariosService.editarManual', () => {
  beforeEach(() => jest.clearAllMocks())

  it('llama a fn_editar_honorario_manual con la observación', async () => {
    mockRpc.mockResolvedValue({
      data: { id: 'h-3', monto: 120000, origen: 'MANUAL', notas: 'Tomó 2 empleados más' },
      error: null,
    })

    const result = await honorariosService.editarManual('uuid-1', 120000, 'Tomó 2 empleados más', 3)

    expect(mockRpc).toHaveBeenCalledWith('fn_editar_honorario_manual', {
      p_cliente_id: 'uuid-1',
      p_monto: 120000,
      p_observacion: 'Tomó 2 empleados más',
      p_frecuencia_meses: 3,
    })
    expect(result.ok).toBe(true)
    if (result.ok) expect(result.data.origen).toBe('MANUAL')
  })

  it('retorna DB_ERROR si la RPC rechaza la edición', async () => {
    mockRpc.mockResolvedValue({
      data: null,
      error: { message: 'La observación es obligatoria' },
    })

    const result = await honorariosService.editarManual('uuid-1', 120000, '')
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.code).toBe('DB_ERROR')
  })
})
