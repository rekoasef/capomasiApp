...
export const fondosService = {
  async getMovimientos(opts?: { desde?: string; hasta?: string; tipo?: string }): Promise<ServiceResult<TFondoMovimiento[]>> {
    let query = supabase.from('fondos_movimientos').select('*').order('fecha', { ascending: false })
  ...
  async registrar(form: TFondoMovimientoForm): Promise<ServiceResult<TFondoMovimiento>> {
    ...
    const { data, error } = await supabase.from('fondos_movimientos').insert(parsed.data).select().single()
    if (error) return { ok: false, error: error.message, code: 'DB_ERROR' }
    return { ok: true, data: data as TFondoMovimiento }
  },
