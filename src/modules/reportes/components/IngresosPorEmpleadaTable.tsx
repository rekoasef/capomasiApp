...
export function IngresosPorEmpleadaTable({ anio, mes }: Props) {
  const { data, isLoading } = useIngresosPorEmpleada(anio, mes)
    ...
    <div className="border-border bg-surface border">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-border divide-y">
          {data.map((r) => (
            <tr key={r.empleada} className="border-border border-b last:border-0">
              <td className="py-2 pr-4 pl-4 text-sm">{r.empleada}</td>
      ...
      <p className="border-border text-muted-foreground border-t px-4 py-2 text-[11px]">
        &quot;Sin asignar&quot; agrupa liquidaciones sin empleada vinculada (ej: honorarios
        mensuales cargados sin indicar quién los generó).
      </p>
    </div>
  )
}
