...
describe('empleadasService.create', () => {
  ...
  it('retorna VALIDATION_ERROR si el nombre es muy corto', async () => {
    const result = await empleadasService.create({ nombre: 'A', tipo_relacion: 'DEPENDENCIA', tipo_comision: 'NINGUNA', activo: true })
    expect(result.ok).toBe(false)
  ...
  it('retorna VALIDATION_ERROR si el tipo_relacion es inválido', async () => {
    ...
    const result = await empleadasService.create({ nombre: 'Victoria', tipo_relacion: 'FREELANCE', activo: true })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('crea la empleada correctamente', async () => {
    ...
    const result = await empleadasService.create({ nombre: 'Victoria', tipo_relacion: 'DEPENDENCIA', tipo_comision: 'NINGUNA', activo: true })
    expect(result.ok).toBe(true)
...
describe('empleadasService.crearLiquidacion', () => {
  ...
  it('retorna VALIDATION_ERROR si el concepto está vacío', async () => {
    const result = await empleadasService.crearLiquidacion({
      empleada_id: UUID, concepto: '', tipo_concepto: 'HABER',
      periodo_mes: 4, periodo_anio: 2026, importe: 100000,
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('retorna VALIDATION_ERROR si el importe es negativo', async () => {
    const result = await empleadasService.crearLiquidacion({
      empleada_id: UUID, concepto: 'Sueldo fijo', tipo_concepto: 'HABER',
      periodo_mes: 4, periodo_anio: 2026, importe: -100,
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('crea el concepto correctamente', async () => {
    const liq = { id: 'l-1', empleada_id: UUID, concepto: 'Sueldo fijo', tipo_concepto: 'HABER', importe: 100000 }
...
describe('empleadasService.getResumenPeriodo', () => {
  ...
  function mockResumenChain(empData: unknown, liqData: unknown, pagosData: unknown) {
    const makeEqChain = (resolved: unknown) => {
      const inner = jest.fn().mockResolvedValue(resolved)
      const outer = jest.fn().mockReturnValue({ eq: inner })
      return { select: jest.fn().mockReturnValue({ eq: outer }) }
    }
    mockFrom
      .mockReturnValueOnce({ select: jest.fn().mockReturnThis(), is: jest.fn().mockResolvedValue({ data: empData, error: null }) })
      .mockReturnValueOnce(makeEqChain({ data: liqData, error: null }))
      .mockReturnValueOnce(makeEqChain({ data: pagosData, error: null }))
  }

  it('calcula neto = haberes - descuentos', async () => {
    const emp = [{ id: UUID, nombre: 'Victoria' }]
    const liq = [
      { empleada_id: UUID, tipo_concepto: 'HABER',     importe: 200000 },
      { empleada_id: UUID, tipo_concepto: 'DESCUENTO', importe: 15000  },
    ]
    mockResumenChain(emp, liq, [])
  ...
  it('calcula saldo = neto - total_pagado', async () => {
    const emp   = [{ id: UUID, nombre: 'Victoria' }]
    const liq   = [{ empleada_id: UUID, tipo_concepto: 'HABER', importe: 100000 }]
    const pagos = [{ empleada_id: UUID, importe: 60000 }]
    mockResumenChain(emp, liq, pagos)
