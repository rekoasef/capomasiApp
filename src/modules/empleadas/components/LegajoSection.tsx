      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" size="sm" variant="outline" onClick={() => { form.reset(); setEditing(false) }}>
              Cancelar
            </Button>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <Section title="Información personal">
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
          <Field label="Nombre"     value={[empleada.nombre, empleada.apellido].filter(Boolean).join(' ')} />
          <Field label="DNI"        value={empleada.dni} />
          <Field label="Nacimiento" value={empleada.fecha_nacimiento ? formatDate(empleada.fecha_nacimiento) : null} />
      ...
      <Section title="Información laboral">
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
          <Field label="Tipo de relación" value={empleada.tipo_relacion === 'DEPENDENCIA' ? 'Relación de dependencia' : 'Por hora'} />
          <Field label="Tipo de comisión" value={TIPO_COMISION_LABEL[empleada.tipo_comision ?? 'NINGUNA']} />
          <Field label="Sueldo fijo"      value={empleada.sueldo_fijo != null ? `$${Number(empleada.sueldo_fijo).toLocaleString('es-AR')}` : null} />
          <Field label="Fecha de ingreso" value={empleada.fecha_ingreso ? formatDate(empleada.fecha_ingreso) : null} />
          <Field label="Estado" value={empleada.activo ? 'Activa' : 'Inactiva'} valueClassName={empleada.activo ? 'text-success' : 'text-muted-foreground'} />
        </dl>
      </Section>
    </div>
  )
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div>
      <p className="mb-3 text-[10px] font-bold tracking-[0.14em] uppercase text-muted-foreground border-b border-border pb-1.5">
        {title}
      </p>
      {children}
    </div>
...
function Field({
  ...
  return (
    <div className={className}>
      <dt className="text-[10px] font-bold tracking-widest uppercase text-muted-foreground">{label}</dt>
      <dd className={`mt-0.5 text-sm ${valueClassName ?? ''} ${!value ? 'text-muted-foreground italic' : ''}`}>
        {value ?? '—'}
      </dd>
    </div>
  )
