...
**Sesión 2026-09-09 (larga):** se recargó `facturacion_historica` con el Excel definitivo de Paola (363 filas, oct-2025 → ago-2026) y se resolvieron los 6 problemas de un audio suyo + los comparativos. Migraciones 0075 a 0078. Tres de los seis venían de la 0064, que sacó el paso "pendiente → pagar" de las compras y dejó cabos sueltos: el endoso de cheques quedó sin salida, los gastos personales bajaban el resultado del estudio, y anular dejaba viva la plata (la caja quedó $886.353,35 abajo, ya corregida). Detalle en `docs/funcional/PEDIDOS_PAOLA.md`.
...
**✅ Cadena del cheque de la cueva — corregida el 2026-09-17 (a pedido de Renzo).** La fila "Compra USD" fusionaba dos pasos distintos en un solo movimiento y, como un movimiento tiene **un solo `tipo_movimiento`**, el efectivo que Paola _gastó_ figuraba como que había entrado. Se rearmó la cadena real en cuatro pasos, todos con los RPC que usa la UI y dentro de una transacción: (1) salida del cheque **68579950 MACRO $1.253.000** por "lo cambié en una cueva" con $1.248.000 de entrada en efectivo — los $5.000 quedan como costo del cambio; (2) borrado de la fila "Compra USD"; (3) la compra de dólares como transferencia con cotización ($1.248.000 → US$ 800, a $1.560); (4) ajuste de saldo de Dólares a **US$ 800**, el arrastre de antes del sistema. Caja resultante: cartera **$2.954.818,92**, dólares **US$ 800**, efectivo **−$466.658,57**.

**⚠️ Pendiente de Paola: el saldo real de la caja de efectivo.** Con la cadena bien registrada el efectivo da **−$466.658,57**, y eso **no es un error de la corrección** — la aritmética cierra. Es que el efectivo también arrastra plata de antes del sistema que nunca se cargó, igual que los dólares; el ingreso mal cargado lo venía tapando. Se cierra con "Ajustar balance" en cuanto ella diga cuánto tiene realmente en la caja. Es el único número de todo esto que no sale de la base.
...
**Migración 0085 (2026-09-17):** los recibos se **editan y se eliminan mientras no estén imputados** (regla de Renzo). Imputado, el camino sigue siendo quitar la imputación o anular; la guardia común es `fn_verificar_recibo_editable`. Editar cambia fecha, medios y notas — **no** el número ni el cliente — y al eliminar **el número no vuelve a la serie**. De paso se tapó un agujero viejo: `fn_anular_recibo` borraba los movimientos de fondos del recibo pero dejaba el cheque `EN_CARTERA`, descuadrando la lista contra la tarjeta. Ahora `fn_liberar_cheques_de_recibo` lo saca si no se movió, y **rechaza la operación** si ya está depositado/endosado. Se agregó auditoría a `recibos_medios` y `cheques`, así que un recibo eliminado se reconstruye desde `audit_log`. **Aplicada en producción.**
...
| `recibos_medios`          | Desglose de medios por recibo    | Todos leen/crean / admin edita                    |
...
### 15.1 Pruebas contra la base de producción

Desde 2026-09-04 el sistema está **en uso real**: Paola trabaja sobre la base de producción todos los días. No hay ambiente de staging.

**Regla: no se cargan datos de prueba en la base real.** Ni recibos, ni clientes, ni gastos "para probar". Cada recibo consume un número de una serie que la clienta le entrega a sus clientes, y cada movimiento ensucia Fondos y los reportes.

Cómo se verifica entonces un cambio que toca la base:

1. **Tests con Jest + mocks** para la lógica de services y funciones puras — es la red principal.
2. **Prueba en seco de la migración**: correrla entera dentro de `BEGIN; ... ROLLBACK;` vía el MCP de Supabase antes de aplicarla. Detecta errores de SQL y de lógica sin dejar rastro.
3. **Prueba funcional del RPC**, también dentro de una transacción revertida. Ojo con dos cosas: `nextval()` **no** se revierte, así que hay que pasar un número de comprobante explícito para no quemar uno de la serie; y `auth.uid()` viene en `NULL` por el MCP.
4. **La prueba de la UI la hace Paola**, en producción, con datos reales suyos. Se le avisa qué probar y en qué orden.

Después de aplicar una migración a producción, verificar con `SELECT` que no se duplicaron movimientos ni quedaron filas huérfanas.
...
| **Facturación histórica del Excel**        | ~~Migrar como `liquidaciones` con `SALDO_INICIAL`~~ — descartado: Paola pidió que sea puramente informativo, sin tocar cuenta corriente ni facturación real. Se migró a tabla aparte `facturacion_historica` (solo columnas A-L de "FC y COBRANZAS", solo admin, sin FKs), con pestaña propia `/facturacion-historica` y filtros por cliente/servicio/generado por/año. **Se recarga entera cada vez que Paola manda una versión nueva del Excel** (no se agregan filas sueltas): el archivo trae correcciones sobre meses ya cargados, así que se borra todo y se vuelve a insertar. Última recarga: 2026-09-09 con "PARA MIGRAR RENZO DEFINITIVO.xlsx", 363 filas (oct-2025 → ago-2026).                                                                                                                                                                                                                                                                                                                                                                                     | ✅ Recargado 2026-09-09 (363 filas)               |
| **Saldo inicial de cuenta corriente real** | Paola lo pidió el 2026-09-03: necesita mandarle al cliente la cuenta corriente con lo que ya debía e imputarle los cobros. Se implementó como botón "Saldo inicial" en la cuenta corriente del cliente (solo admin): crea una liquidación con `tipo_liquidacion = 'SALDO_INICIAL'`, sin comprobante, editable, uno por cliente. Suma al devengado de `v_cuenta_corriente` y es imputable con recibos, pero queda fuera de todas las views de ingresos y de los reportes, así que **no duplica** lo que ya está en `facturacion_historica`. Ver `supabase/migrations/0067_saldo_inicial_cuenta_corriente.sql`. El 2026-09-04 se agregó la otra mitad: el cliente puede entrar **con plata a favor**, que se guarda como un recibo `tipo_pago = 'SALDO_INICIAL'` sin imputar (cuenta en `saldo_a_favor`, imputable a facturas nuevas, sin movimiento de fondos ni cobranza). Un solo saldo inicial por cliente: deuda **o** a favor, se cambia de lado desde el mismo formulario mientras no se le haya imputado nada. Ver `supabase/migrations/0068_saldo_inicial_a_favor.sql`. | ✅ Hecho 2026-09-04                               |
| **Series de recibo A/C eliminadas**        | La migración 0018 partía el recibo por serie: `A-` para lo imputado a Factura A, `C-` para el resto, y dos recibos si un cobro cubría ambas. **Ese corte nunca se activó en la práctica**: solo corría al imputar en el mismo momento de cargar el recibo, y Paola carga el recibo suelto y lo imputa después con `fn_imputar_recibo`, que nunca miró la serie. Al 2026-09-08 los 11 recibos reales eran serie C y 7 estaban imputados a facturas FC_A; `seq_recibo_a` nunca emitió un recibo. Renzo decidió sacar el corte (migración 0073) y dejar una sola numeración con prefijo `C-`, para no cortar la continuidad de los recibos ya entregados. `seq_recibo_a` y `fn_serie_recibo_de_tipo_comprobante` quedaron en la base sin uso. **Pendiente:** Paola nunca confirmó si el recibo debe llevar la misma letra que la factura. Si su contador dice que sí, hay que reponer el corte, tapar el agujero de `fn_imputar_recibo` (hoy deja imputar un recibo C a una factura A sin avisar) y resolver los 7 recibos ya emitidos.                                           | ⚠️ Decisión tomada sin confirmación de la clienta |
| **Comparativos con facturación migrada**   | Reportes lee `v_ingresos_mensuales_con_historico`, `v_ingresos_por_tipo_mes_con_historico` y `v_ingresos_por_empleada_mes_con_historico` (migraciones 0077/0078), que unen `facturacion_historica` con las liquidaciones reales. Se puede sin doble conteo porque el corte es limpio: la histórica termina el 2026-08-31 y las liquidaciones arrancan el 2026-09-01. **El Dashboard sigue usando las vistas sin histórico a propósito** — nunca se migraron los gastos históricos, y mezclar ingresos viejos con gastos nuevos daría una ganancia falsa. Los 11 "SALDO INICIAL" del Excel se excluyen (no son facturación). El trabajo compartido ("LUCIANA + VICTORIA") se reparte mitad y mitad. Los nombres de empleada se normalizan con `upper(btrim())` en los dos lados porque `empleadas.nombre` tiene espacios al final.                                                                                                                                                                                                                                              | ✅ Hecho 2026-09-09                               |
