-- ============================================================
-- 0027_fix_comision_puntaje_null_acum.sql
-- Bug: SELECT INTO asigna NULL cuando no existe fila en
-- saldo_puntaje_empleadas, aunque tenga COALESCE en la expresión.
-- Esto hacía que v_puntos_acum = NULL, v_total = NULL,
-- y la condición (NULL >= umbral) nunca era TRUE → comision = 0.
-- Fix: usar subquery escalar que devuelve 0 si no hay fila.
-- ============================================================
...
BEGIN
  ...
  -- Subquery escalar: retorna 0 si no existe fila
  SELECT COALESCE(
  ...
  RETURN QUERY SELECT
    v_puntos_periodo, v_puntos_acum, v_total,
...
BEGIN
  ...
  IF v_total >= v_config.umbral_puntaje THEN
    ...
    INSERT INTO liquidaciones_empleadas (
      ...
      p_empleada_id, 'Comisión por objetivos', 'HABER',
      p_periodo_mes, p_periodo_anio,
      ROUND(v_comision, 2),
